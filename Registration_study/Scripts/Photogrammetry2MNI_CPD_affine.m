...
clear opt Transform
opt.method      = 'affine'; % affine registration
...
pos = Transform.s*pos*Transform.R' + repmat(Transform.t',[size(pos,1) 1]);
        
for i = 1:size(pos,1)
    [vv,ii]  = min(sqrt(sum(((pos(i,:) - ScalpSurfaceMesh.node).^2),2)));
    pos(i,:) = ScalpSurfaceMesh.node(ii,:);
end
...
id       = subj(end);
savefile = fullfile('..','Data',subj,'CPD_affine',['Subj',id,'2MNI.mat']);
save(savefile,'pos','Transform')
