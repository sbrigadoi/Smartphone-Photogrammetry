...
subj    = dir('../Data/Subj*');
transf  = 'ANTs_affine'; %ANTs_non_linear
ras2lps = diag([-1 -1 1 1]);

for ss = 1:length(subj)
    
    pathfile    = fullfile(subj(ss).folder,subj(ss).name,transf,'Sensor_positions',[subj(ss).name,'2MNI.csv']);
    data        = csvread(pathfile, 1, 0);
    pos         =  xform_apply(data,inv(ras2lps));
    ...
    for ii = 1:size(pos,1)
        [vv,iii]  = min(sqrt(sum(((pos(ii,:) - ScalpSurfaceMesh.node).^2),2)));
        pos(ii,:) = ScalpSurfaceMesh.node(iii,:);
    end
